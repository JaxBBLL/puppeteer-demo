...
async function downloadFromPage(key, url, page) {
   ...
   * 所以查找的时候需要 trycatch
   */
  const phoneImgs = ["#spec-list > ul > li.img-hover > img"].concat(
    [...Array(10).keys()].map(
      (i) => `#spec-list > ul > li:nth-child(${i > 0 ? i : 1}) > img`
    )
  );
  const detailImgs = [...Array(30).keys()]
    .map(
      (i) =>
        `#J-detail-content > div:nth-child(3) > img:nth-child(${i > 0 ? i : 1})`
    )
    .concat(
      [...Array(30).keys()].map(
        (i) =>
          `#J-detail-content > div:nth-child(4) > img:nth-child(${
            i > 0 ? i : 1
          })`
      )
    )
    .concat(
      [...Array(30).keys()].map(
        (i) =>
          `#J-detail-content > div:nth-child(5) > img:nth-child(${
            i > 0 ? i : 1
          })`
      )
    )
    .concat([
      "#J-detail-pop-tpl-top-new > div:nth-child(2) > div > img",
